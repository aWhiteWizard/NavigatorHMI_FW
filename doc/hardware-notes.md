...
当软件配置（dts/驱动/内核开关）均正确但触摸无响应时，通过以下要点判断是否为硬件损坏：

- **中断正常但状态寄存器始终零** → 传感器失效。ISR 被触发（`/proc/interrupts` 计数在涨），但读 0x814E 始终 0x00，说明芯片检测到 IRQ 电平变化却无坐标数据——触摸传感层损坏（碎裂/压伤/内部断线）。
- **冷启动后 ID 随机变化** → 内部 flash 损坏。连续 3 次冷启读芯片 ID（0x8140-0x8143），若值飘忽不定（`9147` → `FFFF` → `0000`），芯片已不可靠。
- **原厂系统同样失败** → 确认硬件问题。将触摸屏接到正点原子出厂系统测试，若同样无响应则定案为硬件故障，直接更换。

> 参考：`lcd-display-debug.md` 的「触控硬件故障诊断决策树」含完整排查流程。
