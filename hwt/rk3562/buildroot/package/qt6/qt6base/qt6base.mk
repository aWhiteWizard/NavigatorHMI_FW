...
# 2026-08-15 NavigatorHMI: 强制用 Qt 内置 md4c (3rdparty), 不依赖系统 md4c 包
# (Rockchip fork 缺 md4c 包且缺 INPUT_libmd4c 处理, 默认行为下 cmake
#  find_package(md4c REQUIRED) 失败 -> -DINPUT_libmd4c=qt 用内置实现, 与上游
#  buildroot 无 md4c 包时的 else 分支一致)
# 另补 QT_FIND_ALL_PACKAGES_ALWAYS=OFF (上游 buildroot 有, fork 缺):
#  find-all 模式会无条件 find 所有包 (含 md4c/XCB 等), 缺失即 REQUIRED 报错
# system_pcre2 改 OFF: buildroot 强制 system_pcre2=ON 但 Qt 的 WrapSystemPCRE2
#  find 不到系统 pcre2 (fork 缺 pcre2-16 配置), 用 Qt 内置 pcre2 (功能等价)
QT6BASE_CONF_OPTS += -DINPUT_libmd4c=qt -DQT_FIND_ALL_PACKAGES_ALWAYS=OFF \
